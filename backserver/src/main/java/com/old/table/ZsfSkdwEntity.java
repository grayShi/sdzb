package com.old.table;

import javax.persistence.Basic;
import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Id;
import javax.persistence.Table;

@Entity
@Table(name = "ZsfSkdw", schema = "dbo", catalog = "shaoxiongtest")
public class ZsfSkdwEntity {
    private int id;
    private String skdw;

    @Id
    @Column(name = "id", nullable = false)
    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    @Basic
    @Column(name = "skdw", nullable = true, length = 50)
    public String getSkdw() {
        return skdw;
    }

    public void setSkdw(String skdw) {
        this.skdw = skdw;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;

        ZsfSkdwEntity that = (ZsfSkdwEntity) o;

        if (id != that.id) return false;
        if (skdw != null ? !skdw.equals(that.skdw) : that.skdw != null) return false;

        return true;
    }

    @Override
    public int hashCode() {
        int result = id;
        result = 31 * result + (skdw != null ? skdw.hashCode() : 0);
        return result;
    }
}
