...
const getReportDataList = (condition, order) => axios.post(`/api/sample/list?${condition}`, order);
...
export default {
  ...
  methods: {
    doQuery (searchCondition, page, size, order) {
      this.loading = true;
      return getReportDataList(this.formatParams({ status: 'pre_check', searchCondition, page, size }), order).then(data => {
        this.loading = false;
    ...
    goToCommit () {
      if (this.selectRow.length === 0) {
        ...
        commitSampleById({id: this.selectRow, status: 'pre_approve'}).then(data => {
          if (data.data.code === '000') {
