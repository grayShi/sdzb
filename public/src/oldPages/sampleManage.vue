  ...
  <page-main-body :title="title">
    <div style="width: 95%">
      <el-row style="margin-top: 10px">
        ...
        <el-button type="info" @click="$router.push('/viewReturn')">查看返工记录</el-button>
      </el-row>
      <light-table :data="searchData" :rowSelect="true" :columns="columns" :rowClass="tableRowClassName" style="width: 95%">
        <template slot="QRCode" slot-scope="props" v-if="props.prop.status !== '已撤回'">
  ...
  methods: {
    ...
    goToDetail (type, scope) {
      if (type === '查看样品详细信息') {
        ...
        if (scope.status === '已入库') {
          status = 'in';
        } else if (scope.status === '待出库') {
          status = 'wait';
        } else if (scope.status === '已出库') {
          status = 'out';
        }
        this.$router.push(`/sampleDetail?status=${status}`);
      } else if (type === '查看入库进度') {
